...
def average_main_experiment(results_file="results_per_fold.csv"):
    ...
    grouped_df = (
        df.groupby(["Seed", "Explainer", "Lambda_1", "Lambda_2"])
        .agg(
            {
                "AUP": ["mean", "std"],
                "AUR": ["mean", "std"],
    ...
    average_df[["Seed", "Explainer", "Lambda_1", "Lambda_2"]] = grouped_df[
        ["Seed_", "Explainer_", "Lambda_1_", "Lambda_2_"]
    ]

    for metric in ["AUP", "AUR", "Information", "Entropy"]:
        average_df[f"{metric}"] = grouped_df.apply(
            lambda row: f"{row[f'{metric}_mean']:.2f} ± {row[f'{metric}_std']:.4f}",
            axis=1,
        )
