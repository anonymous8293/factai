...
def plot_mean_attributions(
    ...
    if decay:
        ...
        # Compute weight decays showing which time data was most important
        linear_decay_weights = [
            1 - ((time_horizon - t) / time_horizon) for t in range(1, time_horizon + 1)
        ]
        exp_decay_weights = [
            np.e ** (-(time_horizon - t) / time_horizon)
