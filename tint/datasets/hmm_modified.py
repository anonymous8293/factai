...
class HMM_modified(DataModule):
    ...
    @staticmethod
    def next_state(previous_state, time):          ### changed this, made it symmetrical and t is time now, used to be delta_state
        if previous_state == 1:
            params = 0.95 - float(time)/500.0
        else:
            params = 0.05 + float(time)/500.0
    ...
    def download(
        ...
        train_size: int = 800,
        test_size: int = 200,
        signal_length: int = 50,                     ### original version has 200 here        
        split: str = "train",
    ):
        ...
        for i in range(count):
            ...
            for i in range(signal_length):
                next = self.next_state(previous, i)           #### SWAPPED i<-> delta_state 
                state_n = next
        ...
        with open(
            os.path.join(self.data_dir, file + "features_modified.npz"), "wb"           ### added _modified to all file names
        ) as fp:
            pkl.dump(obj=features, file=fp)
        with open(
            os.path.join(self.data_dir, file + "labels_modified.npz"), "wb"
        ) as fp:
            pkl.dump(obj=labels, file=fp)
        with open(
            os.path.join(self.data_dir, file + "importance_modified.npz"), "wb"
        ) as fp:
            pkl.dump(obj=importance_score, file=fp)
        with open(
            os.path.join(self.data_dir, file + "states_modified.npz"), "wb"
        ) as fp:
            pkl.dump(obj=all_states, file=fp)
        with open(
            os.path.join(self.data_dir, file + "labels_logits_modified.npz"), "wb"
        ) as fp:
    ...
    def preprocess(self, split: str = "train") -> dict:
        ...
        with open(
            os.path.join(self.data_dir, file + "features_modified.npz"), "rb"
        ) as fp:
            features = np.stack(pkl.load(file=fp))
        with open(
            os.path.join(self.data_dir, file + "labels_modified.npz"), "rb"
        ) as fp:
    ...
    def prepare_data(self):
        ...
        if not os.path.exists(
            os.path.join(self.data_dir, "train_features_modified.npz")
        ):
            self.download(split="train")
        if not os.path.exists(
            os.path.join(self.data_dir, "test_features_modified.npz")
        ):
    ...
    def true_saliency(self, split: str = "train") -> th.Tensor:
        ...
        with open(
            os.path.join(self.data_dir, file + "states_modified.npz"), "rb"
        ) as fp:
